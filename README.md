...
The learndoc content resides within the [content](content) directory.
...
```bash
docker build -t puzzle-bbt/learndoc:latest .
```

Run it locally and remove the container when it exits:

```bash
docker run --rm -i -p 8080:8080 puzzle-bbt/learndoc
```
...
```bash
buildah build-using-dockerfile -t puzzle-bbt/learndoc:latest .
```

Run it locally with the following command. Beware that `--rmi` automatically removes the built image when the container stops, so you either have to rebuild it or remove the parameter from the command.

```bash
podman run --rm --rmi --interactive --publish 8080:8080 localhost/puzzle-bbt/learndoc:latest
```
